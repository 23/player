  ...
  function (Player, $, opts) {
    ...
    $this.loadRecommendations = function (overwrite) {
      ...
      if (Player.get("clips").length <= 1) {
        var opts = /-new$/.test(Player.get("recommendationMethod"))
          ? { orderby: "uploaded", order: "desc" }
          : { orderby: "rank", order: "desc" };
        if (/^channel-/.test(Player.get("recommendationMethod")))
          opts["album_id"] = Player.get("video_album_id");
          ...
          function (data) {
            $.each(data.photos, function (i, photo) {
              if (photo.photo_id != Player.get("video_photo_id")) {
                Player.get("clips").push(
                  new PlayerVideo(Player, $, "clip", photo),
                );
              }
            });
    ...
    Player.bind("player:loaded player:browse:loaded", function () {
      ...
      $.each(c, function (i) {
        c[i].browseThumbnailUrl =
          ...
          c[i].token +
          "/" +
          $this.browseThumbnailWidth +
          "x" +
          $this.browseThumbnailHeight +
          "cr/thumbnail.jpg";
      });
      var s = Player.get("streams");
      $.each(s, function (i) {
        s[i].browseThumbnailUrl =
          "/" +
          s[i].thumbnail_tree_id +
          "/" +
          s[i].thumbnail_photo_id +
          "/" +
          s[i].thumbnail_token +
          "/" +
          $this.browseThumbnailWidth +
    ...
    /* GETTERS */
    Player.getter("showBrowse", function () {
      return $this.showBrowse;
    });
    Player.getter("browseMode", function () {
      return $this.browseMode;
    });
    Player.getter("recommendationMethod", function () {
      return $this.recommendationMethod;
    });
    Player.getter("hasRecommendations", function () {
      return Player.get("clips").length + Player.get("streams").length > 1;
    });
    Player.getter("nextVideo", function () {
      return $this.getNextVideo();
    });
    Player.getter("playlistClickMode", function () {
      return $this.playlistClickMode;
    });
    Player.getter("browseThumbnailWidth", function () {
      return $this.browseThumbnailWidth;
    });
    Player.getter("browseThumbnailHeight", function () {
      return $this.browseThumbnailHeight;
    });
    ...
    Player.setter("browseMode", function (bm) {
      $this.browseMode = bm;
      if ($this.browseMode) {
        $this.browseMode = !Player.fire("player:module:overlayactivated", {
          name: "browse",
          prevented: false,
        }).prevented;
      }
      if ($this.browseMode != _prevShow) {
        ...
        Player.set("forcer", {
          type: "persist",
          element: "tray",
          active: $this.browseMode,
          from: "browse",
        });
        $this.container.find(".browse-container").show();
        _browseTimeouts.push(
          setTimeout(function () {
            $this.container
              .find(".browse-container")
              .toggleClass("browse-container-activated", $this.browseMode);
          }, 10),
        );
        _browseTimeouts.push(
          setTimeout(function () {
    ...
    Player.setter("browse_photo_id", function (id) {
      if (
        Player.get("playlistClickMode") == "link" &&
        Player.get("permission_level") != "none"
      ) {
        Player.set("open_photo_id", { pi: id, target: "_blank" });
      } else if (
        Player.get("playlistClickMode") == "top" &&
        Player.get("permission_level") != "none"
      ) {
