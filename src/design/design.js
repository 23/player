...
Player.provide(
  ...
  function (Player, $, opts) {
    ...
    var _setPlayflowPosition = function (nextPosition) {
      ...
      if (!transition.blocked) {
        window.setTimeout(function () {
          ...
          var playflowClasses = [];
          for (var i = 1; i <= 5; i++) {
            if (i != _playflowPosition)
              playflowClasses.push("playflow-position-" + i);
          }
          $("body")
            .addClass("playflow-position-" + _playflowPosition)
            .removeClass(playflowClasses.join(" "));
    ...
    Player.bind("player:settings", function (e) {
      PlayerUtilities.mergeSettings($this, [
        "verticalPadding",
        "horizontalPadding",
        "trayFont",
        "scrubberColor",
    ...
    $this.applyDesignPreferences = function () {
      ...
      if ($this.backgroundColor) {
        css +=
          "html, body, .player-design, #player, .video-display { background-color: " +
          $this.backgroundColor +
          "; }";
      }
      css +=
        ".tray-left .button-container:hover > button { background-color: " +
        $this.scrubberColor +
        "; }";
      css +=
        ".scrubber-buffer, .scrubber-play { background-color: " +
        $this.scrubberColor +
        "; }";
      css +=
        ".volume-slider .volume-level { background-color: " +
        $this.scrubberColor +
        "; }";
      css +=
        ".big-play-button, .button-container > .loop-play-button { background-color: " +
        $this.scrubberColor +
        "; }";
      css +=
        ".big-play-button:hover, .button-container:hover > .loop-play-button { background-color: " +
        $this.hexToRGBA($this.scrubberColor, 0.85) +
        "; }";
      css +=
        ".share-button:hover, .close-button:hover { background-color: " +
        $this.scrubberColor +
        "; }";
      css += ".sharing-info a:hover { color: " + $this.scrubberColor + "; }";
      css +=
        ".protection-password input.submitbutton { background-color: " +
        $this.scrubberColor +
        "; }";
      css +=
        ".protection-password input.submitbutton:hover { background-color: " +
        $this.hexToRGBA($this.scrubberColor, 0.9) +
        "; }";
      css +=
        ".glue-sections-menu:not(.sections-menu-open) .sections-menu-wrap:hover { background-color: " +
        $this.scrubberColor +
        "; }";
      css +=
        ".sections-menu-open #section-menu-toggle:hover { background-color: " +
        $this.scrubberColor +
        "; }";
      css +=
        ".section-item.active:hover { background-color: " +
        hexToRGB($this.scrubberColor, "0.6") +
        "; }";
      css +=
        ".section-item.active { background-color: " +
        hexToRGB($this.scrubberColor, "0.6") +
        "; }";

      // SVG filter
      css +=
        ".tray-right .button-container:hover > button { -webkit-filter: url(#icon_hover); filter: url(#icon_hover); }";
      css +=
        ".subtitle-button-active .cc-active-bar-icon { -webkit-filter: url(#icon_hover); filter: url(#icon_hover); }";
      css +=
        ".audiodescription-button-active .audiodescription-active-icon { -webkit-filter: url(#icon_hover); filter: url(#icon_hover); }";
      css +=
        ".more-button:hover, .download-button:hover { -webkit-filter: url(#icon_hover); filter: url(#icon_hover); }";
      css +=
        ".video-display .controls-360 div:hover { -webkit-filter: url(#icon_hover); filter: url(#icon_hover); }";

      // Text color
      css +=
        "ul.button-menu li:hover button { color: " +
        $this.scrubberColor +
        "; }";
      ...
      // Border color
      css +=
        ".loop-container-cell { border-color: " + $this.scrubberColor + "; }";
    ...
    var _resize = function () {
      ...
      var buttonWidth = 30;
      var r =
        $(".tray-right .button-container > button:visible").length *
          buttonWidth +
        5;
