...
Player.provide(
  ...
  function (Player, $, opts) {
    ...
    /* GETTERS */
    Player.getter("enableSubtitles", function () {
      return $this.enableSubtitles;
    });
    Player.getter("hasSubtitles", function () {
    ...
    Player.getter("locales", function () {
      return $this.locales;
    });
    ...
    Player.getter("subtitleLocale", function () {
      return $this.subtitleLocale;
    });
    Player.getter("subtitleDirection", function () {
      try {
        return (
          Player.get("locales")[Player.get("subtitleLocale")].direction || "ltr"
        );
      } catch (e) {
        return "ltr";
      }
    });
    Player.getter("supportsAudioDescriptions", function () {
      return $this.supportsAudioDescriptions;
    });
    Player.getter("hasAudioDescriptions", function () {
      return $this.supportsAudioDescriptions && $this.hasAudioDescriptions;
    });
    Player.getter("audioDescriptionTracks", function () {
      return $this.audioDescriptionTracks;
    });
    ...
    Player.getter("audioDescriptionLocale", function () {
      return $this.audioDescriptionLocale;
    });
    Player.getter("audioDescriptionLocaleMessages", function () {
      return $this.audioDescriptionLocaleMessages;
    });
    ...
    Player.setter("audioDescriptionTracks", function (adt) {
      ...
      $this.hasAudioDescriptions =
        Object.keys($this.audioDescriptionTracks).length > 0;
        ...
        loadVideoTrackFromApi(
          ...
          function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
              ...
              utterance.rate = 1.2;
              messages.push({
                start: data[i].timestamp_begin,
                end: data[i].timestamp_end,
                queued: false,
                utterance: utterance,
                text: text,
              });
    ...
    if (window.speechSynthesis && true) {
      ...
      Player.bind(
        "player:audiodescriptionsupdated player:audiodescriptionchanged player:video:seeked",
        cancelSpeechQueue,
      );
    ...
    Player.bind(
      "player:video:play play:video:playing player:video:pause player:video:progress player:video:timeupdate player:video:seeked",
      function (e, o) {
        $this.possiblyRender();
      },
    );
    ...
    $this.possiblyRender = function () {
      ...
      if ($this.subtitleStreamType == "video") {
        ...
        if (ve) {
          ...
          } else if (ve.getProgramDate) {
            time = ve.getProgramDate();
          }
        }
      }
        ...
        ve.addEventListener(
          ...
          function () {
            for (var i = 0; i < ve.textTracks.length; i += 1) {
              if (
                ve.textTracks[i].language.substr(0, 2) ==
                  Player.get("subtitleLocale").substr(0, 2) ||
                ve.textTracks[i].language ==
                  Player.get("subtitleLocale").replace(/_/, "-")
              ) {
    ...
    var loadVideoSubtitlesFromApi = function () {
      ...
      if (
        (typeof v.subtitles_p != "undefined" && v.subtitles_p) ||
        includeDraftSubtitles
      ) {
        var query = {
          photo_id: Player.get("video_photo_id"),
          token: Player.get("video_token"),
          include_drafts_p: includeDraftSubtitles,
        };
        ...
        Player.get("api").photo.subtitle.list(
          ...
          function (data) {
            ...
            Player.set(
              "subtitleLocale",
              !!$this.defaultLocale && !!$this.subtitlesOnByDefault
                ? $this.defaultLocale
                : "",
            );
            ...
            Player.set(
              ...
              $this.defaultAudioDescripionLocale,
            );
    ...
    Player.bind("player:subtitlechange", function () {
      if (
        $this.subtitleStreamType != "live" ||
        Player.get("subtitleLocale").length == 0
      ) {
            ...
            Player.get("api").live.transcription.list(
              {
                ...
                token: v.token,
                recent_p: 1,
                locale: Player.get("subtitleLocale"),
              },
              function (data) {
