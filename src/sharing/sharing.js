  ...
  function (Player, $, opts) {
    ...
    var _currentTimeClick = function () {
      ...
      if (_shareCurrentTime) {
        ...
        var url =
          Player.get("videoLink") +
          "/" +
          formatTime(parseInt(Player.get("currentTime")));
    ...
    Player.bind("player:settings", function (e, settings) {
      ...
      $this.podcastLink = absolutize(
        $this.podcastLink || Player.get("url") + "/podcast",
      );
    ...
    Player.getter("showSharing", function () {
      ...
      return (
        typeof $this.showSharing == "undefined" ||
        ($this.showSharing && $this.showSharing != "0")
      );
    });
    ...
    Player.getter("downloadUrl", function () {
      ...
      if (
        typeof v.video_1080p_download != "undefined" &&
        v.video_1080p_download.length > 0 &&
        v.video_1080p_size > 0
      ) {
        return Player.get("url") + "/attachment" + v.video_1080p_download;
      } else if (
        typeof v.video_hd_download != "undefined" &&
        v.video_hd_download.length > 0
      ) {
        return Player.get("url") + "/attachment" + v.video_hd_download;
      } else if (
        typeof v.video_medium_download != "undefined" &&
        v.video_medium_download.length > 0
      ) {
    ...
    var socialLink = function (service) {
      ...
      if (_shareCurrentTime) {
        return (
          Player.get("videoLink") +
          "/" +
          formatTime(Player.get("currentTime")) +
          "/" +
          service
        );
      }
    ...
    Player.getter("facebookLink", function () {
      return socialLink("facebook");
    });
    Player.getter("twitterLink", function () {
      return socialLink("twitter");
    });
    Player.getter("tumblrLink", function () {
      return socialLink("tumblr");
    });
    Player.getter("googleLink", function () {
      return socialLink("google");
    });
    Player.getter("linkedinLink", function () {
      return socialLink("linkedin");
    });
    Player.getter("diggLink", function () {
      return socialLink("digg");
    });
    Player.getter("mailLink", function () {
      return socialLink("mail");
    });
    ...
    Player.setter("showSharing", function (ss) {
      ...
      if ($this.showSharing) {
        $this.showSharing = !Player.fire("player:module:overlayactivated", {
          name: "sharing",
          prevented: false,
        }).prevented;
      }
      if ($this.showSharing != _prevShow) {
        ...
        Player.set("forcer", {
          type: "block",
          element: "tray big-play info",
          from: "sharing",
          active: $this.showSharing,
        });

        // Animate the container in/out
        $this.container.find(".sharing-container").show();
        _sharingTimeouts.push(
          setTimeout(function () {
            $this.container
              .find(".sharing-container")
              .toggleClass("sharing-container-active", $this.showSharing);
          }, 10),
        );
        _sharingTimeouts.push(
          setTimeout(function () {
